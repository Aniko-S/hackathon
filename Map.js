...
function Map() {
  ...
  useEffect(() => {
    const interval = setInterval(async () => {
      ...
      const newImageUrl = `https://live.onlinecamera.net/207szegedomterthumbnail2.jpg?uniq=0.961870666192049${imageVersion
        .toString()
        .split('')
        .reverse()
        .join('')}`
  ...
  return (
    <View style={styles.container}>
      ...
      >
        ...
        {parkingLots.map((point, index) => (
          <Circle
            zIndex={10}
            key={index}
            fillColor={point.weight === 1 ? '#d72e2e' : '#1ea71e'}
            strokeWidth={0}
            center={point}
            radius={2}
          />
